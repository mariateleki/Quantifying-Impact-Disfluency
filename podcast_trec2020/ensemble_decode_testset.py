...
# allows import of utils from upper directory
import sys
sys.path.append("..")

import utils_general

import hier_filtering_testset

import data.processor_testset
...
def decode():
    ...
    MODEL_PATH = utils_general.PATH_TO_PROJECT + "/podcast_trec2020/models/{}.pt"
    model1_path  = MODEL_PATH.format("cued_speech_ensemble3_1")
    model2_path  = MODEL_PATH.format("cued_speech_ensemble3_2")
    model3_path  = MODEL_PATH.format("cued_speech_ensemble3_3")
    ...
    # initialize the hierarchical filtering model
    bart_tokenizer, hier_tokenizer, hier_model =  hier_filtering_testset.intitialize_hier()
    ...
    for t in utils_general.TRF_LIST:
        ...
        for current_file in tqdm(files):
            ...
            # clean the data before passing it to the hierarchical model
            transcript_text = data.processor_testset.clean_episode_description(txt=transcript_text)
            
            # run the hierarchical filtering model
            transcript_text = hier_filtering_testset.filtering_data(transcript_text, bart_tokenizer, hier_tokenizer, hier_model)
            
            # run the ensemble model
            article_input_ids1 = bart_tokenizer.batch_encode_plus([transcript_text],
...
if __name__ == "__main__":
    ...
    print(utils_general.PATH_TO_PROJECT)
    decode()
